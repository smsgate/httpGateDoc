...
```
http://адрес_сервера/ваш_скрипт?date=2014-05-28 05:38:15&prefix=0001&text=test_sms&smsid=543&sender=3443&receiver=79101234567
```
Переменные:
...
После обработки "входных данных" ваш скрипт должен ответить HTTP статусом 200 с текстом **smsid=543** (Уникальный идентификатор СМС). Также есть возможность, в синхронном режиме, делать запрос на ваш сервер для получения текста ответного смс (МТ).
